...
In the fourth line. It should be "g++ -o p7 p7.o circuit.o".
__
...
It does not have a header guard. If it is included more than once, it may cause multiple definitions of the class defined in the header file.

It will cause a compiling error.

Add the following code at the beginning and the end of the header file:
#ifndef CIRCUIT_H
#define CIRCUIT_H
...
#endif
...
double (*fncptr)(double, double, double);__
...
bool is_product_tree(tree_t tree) {
    ...
    if (tree_isEmpty(left) && tree_isEmpty(right)) {
        return true;
    }
    if (tree_isEmpty(left) || tree_isEmpty(right)) {
        return false;
...
int main(int argc, char* argv[]) {
    ...
    if (n < 1) {
        std::cout << "An input value (n) outside its required range (n >= 1)." << std::endl;
        return -1;
    }
    if (b < 0) {
        std::cout << "An input value (b) outside its required range (b >= 0)." << std::endl;
        return -1;
    }
    if (s < 0) {
        std::cout << "An input value (s) outside its required range (s >= 0)." << std::endl;
        return -1;
    }
    print_split(n, b, s);
    return 0;
...
Case 1: n = 3, b = 3, s = 0 (lower bound)
        Output: (0,0,0)
Case 2: n = 1, b = 2, s = 2 (smallest n)
        Output: (2)
Case 3: n = 2, b = 3, s = 6 (upper bound)
        Output: (2,2,2)
Case 4: n = 2, b = 2, s = 5 (out of bound)
...
//          prefixed by the elements in "prefix".
void print_split_helper(int n, int b, int s, list_t prefix) {
    if (n == 0) {
        if (s == 0) {
            list_print(prefix);
        }
    ...
    for (int i = 0; i <= b; ++i) {
        if (i <= s) {
            list_t next_prefix = list_append(prefix, i);
            print_split_helper(n - 1, b, s - i, next_prefix);
        }
    }
}
